...
def create_waa_data(graph, nodes):
    ...
    d = {'one' : fst_column,
        'two' : snd_column}
    dataFrame_waa = pd.DataFrame(d)


    # Aplico la funcion a cada fila
    dataFrame_waa['waa'] = dataFrame_waa.apply (lambda row: apply_waa(row, graph), axis=1)


    return dataFrame_waa


# In[605]:

# Hasta aqui se hace la creacion del grafo
waa_df = create_waa_data(graph, nodes)
print(waa_df)
...
def getKrecommendations(row, waa_df, df_users, k):
    """
    ...
    """
    #column_result = waa_df[waa_df['one'] == 10].sort_values('waa', ascending=False)
    # user_list = df_users[df_users['user_id'] == 6]
    
   
    
    # primero saco los dos dataframes con problemas que se pueden recomendar
    column_result_one = waa_df[waa_df['one'] == row['problem']]
    column_result_two = waa_df[waa_df['two'] == row['problem']]
    
    
    tmp1 = column_result_two['two'].tolist()
    tmp2 = column_result_two['one'].tolist()
    tmp3 = column_result_two['waa'].tolist()
   
    # creo un nuevo df
    df_tmp = pd.DataFrame({'one':tmp1, 'two':tmp2, 'waa': tmp3})
    ...
    column_result_tmp2 = column_result_tmp.sort_values('waa', ascending=False)
     
    tmp1 = column_result_tmp2['one'].tolist()
    tmp2 = column_result_tmp2['two'].tolist()
    tmp3 = column_result_tmp2['waa'].tolist()
    
    # creo un nuevo df     
    column_result = pd.DataFrame({'one':tmp1, 'two':tmp2, 'waa': tmp3})
...
# Aplico la funcion a cada fila 
k = 10
df_new['list_recommendations'] = df_new.apply (lambda row: getKrecommendations(row, waa_df,  df_users, k), axis=1)
print(df_new)
...
def asignwaavalue(row, waa_df):
    """
        Funcion que devuelve una columna con los valores de waa asociados a cada fila
    """
    
    # print(waa_df.one == row['problem'])
    # print(waa_df.two)
    # print(row['recommendation'])
    # print(waa_df.two == row['recommendation'])
    # print((waa_df.one == row['problem']) & (waa_df.two == row['recommendation']))
    
    new_df = waa_df[(waa_df.one == row['problem']) & (waa_df.two == row['recommendation'])]
    
    if new_df.empty:
        new_df = waa_df[(waa_df.two == row['problem']) & (waa_df.one == row['recommendation'])]
    
    return new_df.iloc[0]['waa']


# In[616]:

df_separation['waa'] = df_separation.apply(lambda row: asignwaavalue(row, waa_df), axis=1)
print(df_separation)


# In[617]:

# ahora lo que quiero es ordenar los problemas por cada usuario en funcion de su waa
# primero ordeno por su valor de user y luego por el de waa, de forma que quedan ordenador por su valor waa
df_separation = df_separation.sort_values(by=['user_id', 'waa'], ascending=False)
print(df_separation)
...
# (los tres primeros ya que estan ordenados por waa)
