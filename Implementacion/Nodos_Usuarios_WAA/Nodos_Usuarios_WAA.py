...
def getCommonNeighbors(row, waa_df):
    """
...
dataframe_user_recomend['neighbors'] = dataframe_user_recomend.apply (lambda row: getCommonNeighbors(row, waa_df), axis=1)
...
def getValueSimilarMetric(row, df_users_recommend, graph):
    ...
    return waa_value(row['user_id'], row['neighbors'], graph)
...
# ahora voy a ordenar por el valor de waa agrupando por el usuario
...
f = open("C:/hlocal/TFM/nodos_usuarios4", 'a')
