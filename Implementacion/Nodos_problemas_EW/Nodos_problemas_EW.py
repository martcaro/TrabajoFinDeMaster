...
def getKrecommendations(row, ew_df, df_users, k):
    """
    ...
    """
    #column_result = ew_df[ew_df['one'] == 10].sort_values('ew', ascending=False)
    # user_list = df_users[df_users['user_id'] == 6]
    
    # ordeno los problemas que se pueden recomendar
    column_result = ew_df[ew_df['one'] == row['problem']].sort_values('ew', ascending=False)
...
# Aplico la funcion a cada fila 
k = 1
df_new['list_recommendations'] = df_new.apply (lambda row: getKrecommendations(row, ew_df,  df_users, k), axis=1)
print(df_new)
...
def asignaewvalue(row, ew_df):
    """
        Funcion que devuelve una columna con los valores de ew asociados a cada fila
    """
    
    # print(ew_df.one == row['problem'])
    # print(ew_df.two)
    # print(row['recommendation'])
    # print(ew_df.two == row['recommendation'])
    # print((ew_df.one == row['problem']) & (ew_df.two == row['recommendation']))
    
    new_df = ew_df[(ew_df.one == row['problem']) & (ew_df.two == row['recommendation'])]
    #print(new_df['ew'])
    
    return new_df.iloc[0]['ew']


# In[386]:

df_separation['ew'] = df_separation.apply(lambda row: asignaewvalue(row, ew_df), axis=1)
print(df_separation)


# In[387]:

# ahora lo que quiero es ordenar los problemas por cada usuario en funcion de su ew
# primero ordeno por su valor de user y luego por el de ew, de forma que quedan ordenador por su valor ew
df_separation = df_separation.sort_values(by=['user_id', 'ew'], ascending=False)
print(df_separation)
...
# (los tres primeros ya que estan ordenados por ew)
