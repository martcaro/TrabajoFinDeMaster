...
def getCommonNeighbors(row, jn_df):
    """
...
dataframe_user_recomend['neighbors'] = dataframe_user_recomend.apply (lambda row: getCommonNeighbors(row, jn_df), axis=1)
...
def jn_value(one, two, graph):
    ...
    value = nx.preferential_attachment(graph, [(one, two)])
    
    value_jn = -1
    for u, v, p in value:
        # print(p)
        # itero el iterador, guardando el valor de jn
        value_jn = p
...
def getValueSimilarMetric(row, df_users_recommend, graph):
    ...
    return jn_value(row['user_id'], row['neighbors'], graph)
...
# ahora voy a ordenar por el valor de jn agrupando por el usuario
