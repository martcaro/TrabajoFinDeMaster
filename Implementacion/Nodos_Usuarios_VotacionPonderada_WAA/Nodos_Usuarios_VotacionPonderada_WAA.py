...
def getSimilarUsers(row, waa_df):
    """
    ...
    column_result_one_tmp = waa_df[waa_df['one'] == row['user_id']]
    column_result_one = column_result_one_tmp[column_result_one_tmp['waa'] > 0]
    column_result_two_tmp = waa_df[waa_df['two'] == row['user_id']]
    ...
    if list_waa == []: # sino tiene vecinos en comun, pongo toda la lista de nodos
        ...
        list_waa.remove(row['user_id']) # y elimino el nodo que estoy mirando
...
dataframe_user_recomend['neighbors'] = dataframe_user_recomend.apply (lambda row: getSimilarUsers(row, waa_df), axis=1)
...
def getWeights(row, graph):
    ...
    for elem in neighbors_list:
        ...
        suma = suma + waa_value(user, elem, graph)
...
def getPonderaciones(row, graph):
    ...
    for elem in neighbors_list:
        ...
        peso_enlace = waa_value(user, elem, graph)
...
f = open("C:/hlocal/TFM/vot_pond2", 'a')
