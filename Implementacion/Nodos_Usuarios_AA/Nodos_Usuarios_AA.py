...
def getCommonNeighbors(row, aa_df):
    """
...
dataframe_user_recomend['neighbors'] = dataframe_user_recomend.apply (lambda row: getCommonNeighbors(row, aa_df), axis=1)
...
def getValueSimilarMetric(row, df_users_recommend, graph):
    ...
    return aa_value(row['user_id'], row['neighbors'], graph)
...
# ahora voy a ordenar por el valor de aa agrupando por el usuario
