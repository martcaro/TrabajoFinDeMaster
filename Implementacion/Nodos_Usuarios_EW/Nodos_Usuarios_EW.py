...
def getCommonNeighbors(row, ew_df):
    """
    ...
    column_result_one_tmp = ew_df[ew_df['one'] == row['user_id']]
    column_result_one = column_result_one_tmp[column_result_one_tmp['ew'] > 0]
    column_result_two_tmp = ew_df[ew_df['two'] == row['user_id']]
    ...
    if list_ew == []: # sino tiene vecinos en comun, pongo toda la lista de nodos
        list_ew = graph.nodes()
        list_ew.remove(row['user_id']) # y elimino el nodo que estoy mirando
...
dataframe_user_recomend['neighbors'] = dataframe_user_recomend.apply (lambda row: getCommonNeighbors(row, ew_df), axis=1)
...
def getValueSimilarMetric(row, df_users_recommend, graph):
    ...
    return ew_value(row['user_id'], row['neighbors'], graph)
...
# ahora voy a ordenar por el valor de ew agrupando por el usuario
