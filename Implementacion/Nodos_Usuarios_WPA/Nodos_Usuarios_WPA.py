...
def create_wpa_data(graph, nodes):
    ...
    # columna con el valor de wpa para ese par de problemas
    fst_column = list()
    snd_column = list()
    ...
    d = {'one' : fst_column,
        'two' : snd_column}
    dataFrame_wpa = pd.DataFrame(d)


    # Aplico la funcion a cada fila
    dataFrame_wpa['wpa'] = dataFrame_wpa.apply (lambda row: apply_wpa(row, graph), axis=1)


    return dataFrame_wpa
...
def getCommonNeighbors(row, wpa_df):
    """
    ...
    column_result_one_tmp = wpa_df[wpa_df['one'] == row['user_id']]
    column_result_one = column_result_one_tmp[column_result_one_tmp['wpa'] > 0]
    column_result_two_tmp = wpa_df[wpa_df['two'] == row['user_id']]
    ...
    if list_wpa == []: # sino tiene vecinos en comun, pongo toda la lista de nodos
        ...
        list_wpa.remove(row['user_id']) # y elimino el nodo que estoy mirando
...
dataframe_user_recomend['neighbors'] = dataframe_user_recomend.apply (lambda row: getCommonNeighbors(row, wpa_df), axis=1)
...
def wpa_value(one, two, graph):
    """
        Funcion que devuelve el valor de weighted preferential attachment
    """
    value_wpa = graph.degree(one, weight="weight") * graph.degree(two, weight="weight")
...
def getValueSimilarMetric(row, df_users_recommend, graph):
    ...
    return wpa_value(row['user_id'], row['neighbors'], graph)
...
# ahora voy a ordenar por el valor de wpa agrupando por el usuario
...
f = open("C:/hlocal/TFM/nodos_usuarios5", 'a')
