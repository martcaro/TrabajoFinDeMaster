...
def getKrecommendations(row, jn_df, df_users, k):
    """
    ...
    """
    #column_result = jn_df[jn_df['one'] == 10].sort_values('jn', ascending=False)
    # user_list = df_users[df_users['user_id'] == 6]
    
   
    
    # primero saco los dos dataframes con problemas que se pueden recomendar
    column_result_one = jn_df[jn_df['one'] == row['problem']]
    column_result_two = jn_df[jn_df['two'] == row['problem']]
    
    
    tmp1 = column_result_two['two'].tolist()
    tmp2 = column_result_two['one'].tolist()
    tmp3 = column_result_two['jn'].tolist()
   
    # creo un nuevo df
    df_tmp = pd.DataFrame({'one':tmp1, 'two':tmp2, 'jn': tmp3})
    ...
    column_result_tmp2 = column_result_tmp.sort_values('jn', ascending=False)
     
    tmp1 = column_result_tmp2['one'].tolist()
    tmp2 = column_result_tmp2['two'].tolist()
    tmp3 = column_result_tmp2['jn'].tolist()
    
    # creo un nuevo df     
    column_result = pd.DataFrame({'one':tmp1, 'two':tmp2, 'jn': tmp3})
...
# Aplico la funcion a cada fila 
k = 10
df_new['list_recommendations'] = df_new.apply (lambda row: getKrecommendations(row, jn_df,  df_users, k), axis=1)
print(df_new)
...
def asignajnvalue(row, jn_df):
    """
        Funcion que devuelve una columna con los valores de jn asociados a cada fila
    """
    
    # print(jn_df.one == row['problem'])
    # print(jn_df.two)
    # print(row['recommendation'])
    # print(jn_df.two == row['recommendation'])
    # print((jn_df.one == row['problem']) & (jn_df.two == row['recommendation']))
    
    new_df = jn_df[(jn_df.one == row['problem']) & (jn_df.two == row['recommendation'])]
    
    if new_df.empty:
        new_df = jn_df[(jn_df.two == row['problem']) & (jn_df.one == row['recommendation'])]
    
    return new_df.iloc[0]['jn']


# In[386]:

df_separation['jn'] = df_separation.apply(lambda row: asignajnvalue(row, jn_df), axis=1)
print(df_separation)


# In[387]:

# ahora lo que quiero es ordenar los problemas por cada usuario en funcion de su jn
# primero ordeno por su valor de user y luego por el de jn, de forma que quedan ordenador por su valor jn
df_separation = df_separation.sort_values(by=['user_id', 'jn'], ascending=False)
print(df_separation)
...
# (los tres primeros ya que estan ordenados por jn)
